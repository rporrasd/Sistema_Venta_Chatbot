...
function loginValidate(loginForm) {
  ...
  if (loginForm.login.value == "") {
    errorMessage += "Correo Electronico Vacío!\n";
    validationVerified = false;
  }
  if (loginForm.password.value == "") {
    errorMessage += "Contraseña Vacío!\n";
    validationVerified = false;
  }
  if (!isValidEmail(loginForm.login.value)) {
    errorMessage += "Correo con formato invalido!\n";
    validationVerified = false;
  }
...
// //function to handle login-form validation
// function PasarelaValidate(PasarelaForm) {
//   var validationVerified = true;
//   var errorMessage = "";

//   if (loginForm.Titular.value == "") {
//     errorMessage += "Correo Electronico Vacío!\n";
//     validationVerified = false;
//   }
//   if (!validationVerified) {
...
function registerValidate(registerForm) {
  ...
  if (registerForm.fname.value == "") {
    errorMessage += "Nombres Vacío!\n";
    validationVerified = false;
  }
  if (registerForm.lname.value == "") {
    errorMessage += "Apellidos Vacío!\n";
    validationVerified = false;
  }
  ...
  if (registerForm.cpassword.value == "") {
    errorMessage += "Confirmación de contraseña Vacío!\n";
    validationVerified = false;
  }
  ...
  if (registerForm.question.selectedIndex == 0) {
    errorMessage += "Tipo de documento Vacío!\n";
    validationVerified = false;
  }
  if (registerForm.answer.value == "") {
    errorMessage += "Número de documento Vacío!\n";
    validationVerified = false;
  } else if (registerForm.answer.length() > 8) {
    errorMessage += "Número de documento tiene mas de 8 dígitos!\n";
    validationVerified = false;
  }
  if (registerForm.fono.value == "") {
    errorMessage += "Teléfono Vacío!\n";
    validationVerified = false;
  }
  if (registerForm.cDirección.value == "") {
    errorMessage += "Dirección Vacío!\n";
    validationVerified = false;
  }
  if (!validationVerified) {
    alert(errorMessage);
...
function passwordResetValidate(resetForm) {
  ...
  if (resetForm.email.value == "") {
    errorMessage +=
      "Please enter your account email! We need your email in order to reset your password.\n";
    validationVerified = false;
  }
...
function passwordResetValidate_2(resetForm) {
  ...
  if (resetForm.answer.value == "") {
    errorMessage +=
      "Please enter your security answer to your provided security question.\n";
    validationVerified = false;
  }
...
function doTotals(form) {
  ...
  for (var i = 0; PRODUCT_ABBRS[i]; i++) {
    ...
    total +=
      parseFloat(cur_field.value) *
      parseFloat(form.elements[PRODUCT_ABBRS[i] + "_price"].value);
  }
...
function finalCheck(orderForm) {
  ...
  if (orderForm.total.value == "") {
    errorMessage +=
      "Total has not been calculated! Please provide first the quantity.\n";
    validationVerified = false;
  }
  if (!validationVerified) {
    alert(errorMessage);
...
function ratingValidate(ratingForm) {
  ...
  if (ratingForm.food.selectedIndex == 0) {
    errorMessage +=
      "Please select the food. This information is necessary in order to serve you better.\n";
    validationVerified = false;
  }
  if (ratingForm.scale.selectedIndex == 0) {
    errorMessage +=
      "Please select the scale. This information is necessary in order to serve you better.\n";
    validationVerified = false;
  }
  if (!validationVerified) {
    alert(errorMessage);
...
function getQuantity(int) {
  ...
  xmlhttp.open("GET", "update-quantity.php?cant_id=" + int, true);
  xmlhttp.send();
}
